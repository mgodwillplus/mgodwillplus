...
(function () {
  ...
  function drivePreview(url) {
    ...
    // Prefer the existing catalogue helper when it exists, but fall back
    // to a direct Google Drive preview URL if it does not.
    ...
    if (match && /drive\.google\.com|drive\.usercontent\.google\.com/i.test(value)) {
  ...
  function driveThumb(url) {
    if (typeof getDriveThumbnailUrl === "function") return getDriveThumbnailUrl(url);
    const match = String(url || "").match(/\/file\/d\/([^/]+)/);
    return match ? `https://drive.google.com/thumbnail?id=${match[1]}&sz=w1200` : url;
  }

  function mediaThumb(item) {
    if (typeof getMediaThumbnail === "function") return getMediaThumbnail(item);
    return item.thumbnail || "";
  }

  function mediaPrimaryUrl(item) {
    if (typeof getPrimaryMediaUrl === "function") return getPrimaryMediaUrl(item);
    return Array.isArray(item.links) ? item.links[0] : item.link;
  }

  function mediaLinks(item) {
    if (typeof getMediaUrls === "function") return getMediaUrls(item);
  ...
  function safeImage(url, fallbackText = "MGODWILL+") {
    ...
    return `<div class="media-image-fallback">${escapeHtml(fallbackText)}</div>`;
  }
    ...
    host.innerHTML = `
      <footer class="site-footer">
        <div class="footer-main">
          <div>
            <div class="footer-logo">MGODWILL<span>+</span></div>
            <p>A personal media universe for films, music, voices, ideas and stories.</p>
          </div>
  ...
  function mediaCard(item, options = {}) {
    const compact = options.compact ? " media-card-compact" : "";
    const thumb = mediaThumb(item);
    const category = item.category || typeLabel(item);

    return `
      <article class="media-card${compact}" data-media-id="${escapeHtml(item.id)}">
        <button class="media-card-button" type="button"
          ...
          <div class="media-card-image">
            ...
            <span class="media-card-type">${escapeHtml(typeLabel(item))}</span>
          </div>
  ...
  function renderRail(target, items, emptyMessage = "Nothing here yet.") {
    if (!target) return;
  ...
  function renderHome() {
        ...
        featured.innerHTML = `
          <div class="featured-media">
            ...
            <div class="featured-content">
              <span class="eyebrow">MGODWILL+ FEATURED</span>
              <h1>${escapeHtml(item.title)}</h1>
              <p>${escapeHtml(item.description || "")}</p>
              <div class="button-row">
                <a class="btn btn-primary" href="${escapeHtml(pageUrl("player.html", { id: item.id }))}">▶ Watch</a>
                <a class="btn btn-secondary" href="${escapeHtml(pageUrl("player.html", { id: item.id }))}">More Info</a>
              </div>
            </div>
          </div>
        `;
    ...
    renderRail($("#home-watch"), media.filter((item) => item.type === "Video").slice(0, 10));
    renderRail($("#home-music"), media.filter((item) => item.type === "Song").slice(0, 10));
    renderRail($("#home-podcasts"), media.filter((item) => String(item.type).toLowerCase() === "podcast").slice(0, 10));

    const mavuno = categoryItems("Mavuno Church");
    renderRail($("#home-mavuno"), mavuno);

    const aurora = categoryItems("Aurora Newdawn");
    renderRail($("#home-aurora"), aurora);

    const journalTarget = $("#home-journal");
    if (journalTarget) renderJournal(journalTarget, journal.slice(0, 6));
  }
  ...
  function renderWatch() {
    ...
    const videos = media.filter((item) => String(item.type).toLowerCase() === "video");
    let active = "All";

    function draw() {
      const selected = active === "All"
        ? videos
        : videos.filter((item) => item.category === active);

      grid.innerHTML = selected.length
        ? `<div class="media-grid">${selected.map((item) => mediaCard(item)).join("")}</div>`
    ...
    if (chips) {
      const categories = ["All", ...new Set(videos.map((item) => item.category).filter(Boolean))];

      chips.innerHTML = categories
        .map((category) => `
          <button class="category-chip${category === "All" ? " active" : ""}"
                  type="button"
      ...
      $$("[data-watch-category]", chips).forEach((button) => {
        button.addEventListener("click", () => {
          active = button.getAttribute("data-watch-category") || "All";
          $$("[data-watch-category]", chips).forEach((chip) => chip.classList.remove("active"));
  ...
  function renderMusic() {
    ...
    const songs = media.filter((item) => String(item.type).toLowerCase() === "song");

    grid.innerHTML = songs.length
      ? `<div class="media-grid music-grid">${songs.map((item) => mediaCard(item, { compact: true })).join("")}</div>`
      : `<div class="empty-state">No music has been added yet.</div>`;
  ...
  function renderPodcasts() {
    ...
    const podcasts = media.filter(
      (item) => String(item.type).toLowerCase() === "podcast"
    );

    grid.innerHTML = podcasts.length
      ? `<div class="media-grid">${podcasts.map((item) => mediaCard(item)).join("")}</div>`
      : `<div class="empty-state">No podcasts have been added yet.</div>`;
  ...
  function getEmbedUrl(url) {
    ...
    /* GOOGLE DRIVE */
    if (
      /drive\.google\.com\/file\/d\//i.test(value) ||
      /drive\.google\.com.*[?&]id=/i.test(value) ||
      /drive\.usercontent\.google\.com.*[?&]id=/i.test(value)
    ) {
